    ...
    <div className="min-h-screen gradient-bg flex items-center justify-center p-4">
      ...
      <div className="glass-card p-8 sm:p-10 w-full max-w-md relative animate-fade-in">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-text-secondary mt-1">Sign in to your Prepzify account</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button type="submit" disabled={loading} className="btn btn-primary w-full py-3 text-base" id="login-submit">
            {loading ? <div className="spinner w-5 h-5 border-2" /> : <>Sign In <FiArrowRight /></>}
          </button>
        </form>

        <p className="text-center text-text-secondary mt-6 text-sm">
          Don&apos;t have an account?{' '}
          <Link to="/signup" className="text-primary-light hover:text-primary font-medium">Create one</Link>
        </p>
      </div>
    </div>
  );
