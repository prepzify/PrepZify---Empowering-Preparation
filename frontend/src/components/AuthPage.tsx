...
import { Mail, Lock, User, ArrowRight, Github, Chrome, AlertCircle, Eye, EyeOff, RefreshCcw, Sparkles } from 'lucide-react';
...
export default function AuthPage() {
  ...
  useEffect(() => {
    ...
    const checkRedirect = async () => {
      ...
      if (user) {
        navigate(selectedPlan ? `/landing?plan=${selectedPlan}` : '/');
      }
    };
    checkRedirect();
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        if (!showSignupOtp) {
          // Trigger OTP Generation & show the OTP verification card
        ...
        if (enteredSignupOtp !== signupOtpCode) {
          setSignupOtpError('Invalid confirmation OTP code. Please check and try again.');
          setLoading(false);
          return;
        }
    ...
    <div className="min-h-screen bg-background flex flex-col md:flex-row overflow-hidden italic selection:bg-primary/30">
      ...
      <div className="flex-1 flex items-center justify-center p-6 md:p-12">
        <div className="w-full max-w-md space-y-10">
           ...
           {showSignupOtp ? (
             <form onSubmit={handleAuth} className="space-y-6 animate-in fade-in duration-300">
                <div className="space-y-3 text-center md:text-left">
                ...
                {signupOtpError && (
                  <div className="p-4 bg-error-container/10 border border-error/20 rounded-xl flex items-center gap-3 text-error text-xs font-bold">
                    <AlertCircle className="w-4 h-4 shrink-0" />
                    {signupOtpError}
                  </div>
                )}

                <div className="p-4 bg-primary/10 border border-primary/20 rounded-2xl text-center text-xs font-bold text-primary flex items-center justify-center gap-2">
                  <Sparkles className="w-4 h-4" />
                  <span>[Sandbox Alert] Verification OTP is: <span className="font-mono text-sm underline select-all">{signupOtpCode}</span></span>
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant pl-1 block text-center">Enter Verification Code</label>
                  <input 
                    type="text" 
                    maxLength={6}
                    required 
                    ...
                    className="w-full bg-surface-container border border-outline-variant rounded-2xl py-4 text-center text-lg font-mono tracking-[0.4em] font-bold focus:outline-none focus:border-primary transition-all"
                  />
                </div>
                ...
                <div className="flex gap-3">
                  <button 
                    type="button" 
                    onClick={() => { setShowSignupOtp(false); setSignupOtpError(''); }}
                    className="flex-1 py-4 border border-outline-variant text-[11px] font-bold uppercase tracking-widest rounded-2xl hover:bg-surface-container-high transition-colors text-center block cursor-pointer"
                  >
                    Back
                  </button>
                  <button 
                ...
                <button 
                  type="submit" 
                  disabled={loading || enteredSignupOtp.length !== 6}
                  className="w-full bg-primary text-on-primary py-4 rounded-2xl text-[11px] font-bold uppercase tracking-widest flex items-center justify-center gap-2 hover:brightness-110 active:brightness-90 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-xl shadow-primary/20 cursor-pointer"
                >
